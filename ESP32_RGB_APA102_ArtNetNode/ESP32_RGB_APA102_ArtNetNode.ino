...
const uint8_t maxPixelsPerUniverse = 170;
...
const char *ssid = "ArtNetConfigAP";
...
void setup() 
{
  ...
  if(systemMode==0)
  {
    ...
    if(totalPixels>0)
    {
      ledStrip.init(totalPixels, 4, 255);
      for(uCount=0; uCount<totalPixels; uCount++)
      {
        ledStrip.setPixel(uCount, blankColour);
      }
	  SPI.writeBytes(ledStrip.LEDs, ledStrip._frameLength);
    }
...
{   
  ...
  {
    //Do nothing render task takes care of pixel render
	SPI.writeBytes(ledStrip.LEDs, ledStrip._frameLength);
  }
}
...
void pollDMX(AsyncUDPPacket &packet)
{
     ...
     if(packetSize==maxPacketBufferSize)
     {
        ...
        for(uCount=0; uCount<numberOfDMXUniverses; uCount++)
        {
          ...
          {
            //Serial.printf("\r\nGot Data for U[%d]S[%d]",universePart,subnetPart );
          ...
          {
           Serial.printf("\r\nU[%d]\tS[%d]\r\n", universePart, subnetPart);
           Serial.printf("\r\nIGNORED Data for U[%d]S[%d]\r\n",packetBuffer[14],packetBuffer[15] );
           for(testCounter=0; testCounter<16; testCounter++)
           {
            Serial.printf("%d->[%d]\r\n", testCounter, packetBuffer[testCounter] );
           }
          }
          */
        }
...
void writeToPixels(uint8_t universeId)
{
  ...
  {
    memcpy(tempColour, artNetData.data+dmxIndex, 3);
    ledStrip.setPixel(pStart+pIndex, tempColour);
    dmxIndex+=3;
  }
}
