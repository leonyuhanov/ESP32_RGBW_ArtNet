...
{  
  uint8_t blackOutCol[4] = {0,0,0,0};
  
  _numLEDs = numLEDs;
  _LEDframeLength = _numLEDs*4;
  _NeoBitsframeLength = _LEDframeLength*4;
  
  LEDs = new uint8_t[_LEDframeLength];
  neoBits = new uint8_t[_NeoBitsframeLength];
...
void NeoViaSPI::setPixel(short int pixelIndex, uint8_t *pixelColour)
{
  _counter = 4*pixelIndex;
  LEDs[ _counter ] = pixelColour[1];
  LEDs[ _counter + 1 ] = pixelColour[0];
  LEDs[ _counter + 2 ] = pixelColour[2];
  LEDs[ _counter + 3 ] = pixelColour[3];
}

void NeoViaSPI::getPixel(short int pixelIndex, uint8_t *pixelColour)
{
   _counter = 4*pixelIndex;
   pixelColour[0] = LEDs[ _counter + 1 ];
   pixelColour[1] = LEDs[ _counter ];
   pixelColour[2] = LEDs[ _counter + 2 ];
   pixelColour[3] = LEDs[ _counter + 3 ];
}
...
void NeoViaSPI::encode()
{
  _nCounter = 0;
  //Pixel Loop
  for(_counter=0; _counter<_LEDframeLength; _counter+=4)
  {
    //Colour Loop
    for(_colCounter=0; _colCounter<4; _colCounter++)
    {
      //Bit Loop
